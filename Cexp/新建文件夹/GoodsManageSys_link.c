...
{
    FILE *fp = fopen("C:\Users\房文宇\Desktop\新建文件夹\goodsinfo.txt", "r");
    //file not exist
    if (!fp) {
        printf("商品信息初始化文件不存在！程序将为您新建一个.\n");
        FILE *fp = fopen("C:\Users\房文宇\Desktop\新建文件夹\goodsinfo.txt", "w");
        fclose(fp);
...
//1
   FILE *fp;              //修改在当前PC下的路径 
   fp = fopen("C:\Users\房文宇\Desktop\新建文件夹\goodsinfo.txt", "w");
   fclose(fp);
//2
	DestroyGoods(*L);     //清空链表 
	GoodsList *TMP ;
